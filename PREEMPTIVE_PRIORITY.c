...
// FUNCTION: PreemptivePriorityScheduling(processes, numberOfProcesses)
void PreemptivePriorityScheduling(process *p, int nop){
	int i, currentTime=0, completed=0, timeFactor, pno, timeStamps[1000], k=0, visited=1;
...
// END OF PreemptivePriorityScheduling() FUNCTION
...
int main(){
	...
	PreemptivePriorityScheduling(p, nop);
